...
def H2_cod(zone1, zone2, cod1, cod2, start_time, duration, sampleFreq, slideWindow=2, step=1, maxlag=0.1, L=7, HP=0,
           LP=0):
    h2_bwt = []
    h2_bwt_max = []
    h2_bwt_median = []
    ...
    for i in range(len(zone1)):
        ...
        for j in range(lz2):
            yi = xi + j
            h2_bwt.append(
                H2_filter([zone1[i], zone2[j]], start_time, duration, sampleFreq, slideWindow, step, maxlag, L, HP, LP))
...
def H2_bwt(zone1, zone2, start_time, duration, sampleFreq, slideWindow=2, step=1, maxlag=0.1, L=7, HP=0, LP=0):
    ...
    for i in range(len(zone1)):
        h2_bwt.append(
            H2_filter([zone1[i], zone2[i]], start_time, duration, sampleFreq, slideWindow, step, maxlag, L, HP, LP))
...
def H2_bwt_cod(zone1, zone2, cod1, cod2, start_time, duration, sampleFreq, slideWindow=2, step=1, maxlag=0.1, L=7, HP=0,
               LP=0):
    h2_bwt = []
    h2_bwt_max = []
    h2_bwt_median = []
...
def h2_in_suit(zone, cod, label, start_time, duration, sampleFreq, slideWindow=2, step=1, maxlag=0.1, L=7, HP=0, LP=0):
    ...
    for i in range(lz - 1):
        for j in range(i + 1, lz):
            h2_bwt.append(
                H2_filter([zone[i], zone[j]], start_time, duration, sampleFreq, slideWindow, step, maxlag, L, HP, LP))
    ...
    for i in range(len(h2_bwt_median)):
        plt.text(i + 1, h2_per_max[i] + 0.01, np.round(h2_bwt_median[i], 3), ha='center', va='bottom',
                 fontsize=15)  # ha：水平对齐，va：垂直对齐
    return h2_bwt, h2_bwt_max, h2_bwt_median, np.median(h2_bwt_median), codistance


def h2_btw_suit0(zone1, zone2, cod1, cod2, label, start_time, duration, sampleFreq, slideWindow=2, step=1, maxlag=0.1,
                L=7, HP=0, LP=0):
    h2_bwt = []
    h2_bwt_max = []
    h2_bwt_median = []
...
def violin(vlist, label, title):
    ...
    color = ['DeepSkyBlue', 'DarkTurquoise', 'MediumSpringGreen', 'GreenYellow', 'Yellow', 'Orange', 'SandyBrown',
             'LightPink', 'Violet', 'BlueViolet']
    i = 0
    for pc in vp['bodies']:
...
def H2_direction(h2):
    ...
    不加权（百分比）
    '''
    pnum = len(h2[0])  # 时间点的数量
    h2_direct = [0] * pnum
    for pi in range(pnum):
        if h2[0][pi][0, 1] >= h2[0][pi][1, 0]:
            if h2[1][pi][0, 1] > 0:
                h2_direct[pi] = -1
            elif h2[1][pi][0, 1] < 0:
                h2_direct[pi] = 1
    ...
    return (h2_direct.count(1) - h2_direct.count(-1)) / pnum


def H2_direction_modify(h2):
    '''
     加权
    '''
    pnum = len(h2[0])  # 时间点的数量
    ...
    for pi in range(pnum):
        if h2[0][pi][0, 1] >= h2[0][pi][1, 0]:
            if h2[1][pi][0, 1] > 0:
                # h2_direct[pi] = -1
                negative.append(h2[0][pi][0, 1])
            elif h2[1][pi][0, 1] < 0:
                # h2_direct[pi] = 1
                positive.append(h2[0][pi][0, 1])
        else:
            if h2[1][pi][1, 0] > 0:
                # h2_direct[pi] = 1
                positive.append(h2[0][pi][1, 0])
            elif h2[1][pi][1, 0] < 0:
                # h2_direct[pi] = -1
                negative.append(h2[0][pi][1, 0])
    sp = sum(positive)
    sn = sum(negative)
    return (sp - sn) / (sp + sn), sp, sn


def h2_btw_suit(zone1, zone2, cod1, cod2, label, start_time, duration, sampleFreq, slideWindow=2, step=1, maxlag=0.1,
                L=7, HP=0, LP=0):
    h2_bwt = []
    h2_bwt_max = []
    h2_bwt_median = []
    ...
    for i in range(len(zone1)):
        for j in range(lz2):
            h2_bwt.append(
                H2_filter([zone1[i], zone2[j]], start_time, duration, sampleFreq, slideWindow, step, maxlag, L, HP, LP))
            h2_bwt_max.append(H2_max(h2_bwt[-1][0], h2_bwt[-1][1]))
            h2_bwt_direct.append(H2_direction(h2_bwt[-1]))
            a, b = np.percentile(h2_bwt_max[-1][0], [50, 100])
            h2_bwt_median.append(a)
            h2_per_max.append(b)
            codistance.append(points_dist(cod1[i], cod2[j]))
    ...
    for i in range(len(h2_bwt_median)):
        plt.text(i + 1, h2_per_max[i] + 0.01, np.round(h2_bwt_median[i], 3), ha='center', va='bottom',
                 fontsize=15)  # ha：水平对齐，va：垂直对齐
    return h2_bwt, h2_bwt_max, h2_bwt_median, np.median(h2_bwt_median), codistance, h2_bwt_direct


def violinbox_btw(vlist, maxlag, title, way):
    plt.figure(figsize=(10, 6))
    vp = plt.violinplot(vlist, showmedians=True)  # data里是元素（如data1）还不允许是Series
    color = ['DeepSkyBlue', 'Yellow', 'Violet']
    i = 0
    for pc in vp['bodies']:
        pc.set_facecolor(color[i])
        pc.set_edgecolor('black')
    ...
    plt.title(title, fontsize=18)
    plt.grid()
    plt.xticks(list(range(1, 4)), ['EZ-PZ', 'EZ-NIZ', 'PZ-NIZ'], fontsize=10)  # 只能这么设置label
    plt.xlabel("最大延迟时间：{}s".format(maxlag), fontsize=12)
