"""
此脚本实现计算脑电信号某个signal的 2D, 1D, 0D bicoherence

python bicoherence.py zhangjie-10sz.edf C110 100 110 5 6

本程序可以传入4至6个参数，每个参数之间用空格隔开：
参数1：edf文件所在路径，如 zhangjie-10sz.edf
参数2：所选电极名，如 C110
参数3：所选起始时间，如 100
参数4：所选终止时间，如 110
参数5（可选）：所选子波f1频率（可为小数），如 5.3
参数6（可选）：所选子波f2频率（可为小数），如 6

Calculate 2D, 1D, 0D bicoherence, bispectrum, polycoherence and polyspectrum
Reference： https://github.com/trichter/polycoherence
"""
...
try:
	import sys
	if len(sys.argv) < 5:
		raise Exception("至少需要4个参数，您只传入了%d个" % (len(sys.argv)-1))
		
	edf_file = sys.argv[1]
	select_signal = sys.argv[2]
	...
	fs = round(f.getNSamples()[0] / f.getFileDuration())  # f.getSampleFrequency(signal_labels.index('C110'))
	start_index = int(start_time*fs)
	end_index = int(end_time*fs)
	
	t = np.arange(0, f.getNSamples()[0]/fs ,1/fs)[start_index:end_index]
	N = t.size
	
	print("总时间：%s秒\n频率：%s\n起始时间下标：%s\n终止时间下标：%s" % (f.getFileDuration(), fs, start_index, end_index))
	
	row_signal = f.readSignal(signal_labels.index(select_signal))
	signal = (row_signal-np.mean(row_signal))[start_index:end_index]
	kw = dict(nperseg=N // 10, noverlap=N // 20, nfft=next_fast_len(N // 2))
	
	_plot_signal(t, signal)
	plt.suptitle('%s signal' % select_signal)
	
	result1 = polycoherence(signal, fs, **kw)
	plot_polycoherence(*result1)
	plt.suptitle('bicoherence')
	
	if len(sys.argv) == 6:
		f1 = float(sys.argv[5])
		result = polycoherence(signal, fs, f1, dim=1, **kw)
		_plot_polycoherence_1d(*result)
		plt.suptitle('bicoherence for %sHz' % sys.argv[5])
		
	if len(sys.argv) == 7:
		f1 = float(sys.argv[5])
		f2 = float(sys.argv[6])
		print('bicoherence for f1=%sHz, f2=%sHz:' % (sys.argv[5], sys.argv[6]),
          polycoherence(signal, fs, f1, f2, dim=0, **kw))
