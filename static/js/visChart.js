...
function violinBwt() {
    ...
    // Read the data and compute summary statistics for each specie
     ...
     var dataFilter = download_FC_data.map(function(d){return d3.median(d[2]) });  //download_FC_data:["zone", "electrodes", "h2", "nwd", "wd"]
    ...
    var max_y = max_h2 > 1 ? 1 : max_h2;
      ...
      var histogram = d3.histogram()
            .domain(y.domain())
            .thresholds(y.ticks(violin_bins_slider.getValue()))    // Important: how many bins approx are going to be made? It is the 'resolution' of the violin plot
            .value(d => d);
      ...
      var sumstat = d3.nest()  // nest function allows to group the calculation per level of a factor:分层地分组数组元素。
        ...
        .rollup(function(d) {   // For each key..
          input = d.map(function(g) { return d3.median(g[2]);});    // Keep the variable called Sepal_Length
          bins = histogram(input);   // And compute the binning on it.
          return(bins)
        })
        .entries(download_FC_data); //返回一组键-值元组（其实就是一个字典）
      ...
      // databyType = {};
      let myViolin = svg.selectAll("myViolin")
        .data(sumstat)
      ...
      if($('#violin_status').bootstrapSwitch("state")){
        ...
        // Add individual points with jitter
          ...
          svg.selectAll("indPoints")
            .data(download_FC_data)
            .enter()
            .append("circle")
              .attr("cx", function(d){return(x(d[0]) + x.bandwidth()/2 - Math.random()*jitterWidth )})
              .attr("cy", function(d){return(y(d3.median(d[2])))})
              .attr("r", 4)
              .style("fill", function(d){ return(myColor(d3.median(d[2])))})
              .attr("stroke", "white")
              .on("mouseover", function (d) {
                  //sessionStorage.setItem("dvalue",d.value);
                div_tooltip.transition()
              ...
              .on("click",function (d) {
                  ...
                  var subdata = [];
                  for (i of sub_h2){
                      subdata.push({"electrodes":d[1],"h2":i})
                  }
                  subviolinFC(subdata,d[0],d[1])
              });
      }else{
          myViolin.on("mouseover", function (d) {
              //sessionStorage.setItem("dvalue",d.value);
          ...
          myViolin.append("rect")
              .attr("fill","black")
              .attr("x",x.bandwidth()/2-2)  // 矩形的 (x,y) 是右上角
              .attr("y",function (d) {
                  return y(databyType[d.key][6])
              })
              .attr("height", function (d) {
                  return y(databyType[d.key][5]) - y(databyType[d.key][6]);
              })
              .attr("width", 4);

          myViolin.append("circle")
              .attr("cx", x.bandwidth()/2 )
              .attr("cy", function (d) {
                  return y(databyType[d.key][0])
              })
              .attr("r", 3)
              .style("fill", "red")
              .attr("stroke", "red")
    ...
    chartButton.append("button")
        ...
        .html("加权方向性")
        .on("click",function () {
            show_type_direction = 4;
            directionality(4) // ["zone", "electrodes", "h2", "nwd", "wd"]
        });
    ...
    chartButton.append("button")
        ...
        .html("取消")
        .on("click",function () {
            show_type_direction = 0;
            d3.select("#directionality").selectAll("*").remove();
        });
}

function directionality(type) {
    // 3 表示 "nwd" 4 表示 "wd"
    // set the dimensions and margins of the graph
    var margin = {top: 10, right: 60, bottom: 30, left: 50},
        width = 1000 - margin.left - margin.right,
    ...
    var svg = direction
      ...
      var count=0;
      // directionByType={};
      let myViolinDirection = svg.selectAll("myViolinDirection")
        .data(sumstat)
        .enter()        // So now we are working group per group
        .append("g")
          .attr("transform", function(d){
              ...
              pdata.sort((a,b)=>a-b);  // 一定要注意，排序是按照字符串首字母来的！！！！！！      中值、均值、最大值、最小值、数组长度、Q1、Q3
              directionByType[d.key] = [d3.quantile(pdata,0.5),d3.mean(pdata),pdata[pdata.length - 1],pdata[0],pdata.length,d3.quantile(pdata,0.25),d3.quantile(pdata,0.75)];
              return("translate(" + x(d.key) +" ,0)")
          } ) // Translation on the right to be at the group position
        .on("mouseout", function (d) {
            div_tooltip.transition()
      ...
      if($('#violin_status').bootstrapSwitch("state")) {
            myViolinDirection.on("mouseover", function (d) {
                    ...
                    div_tooltip.html("中值："+formatNum(directionByType[d.key][0])+
                        ...
                        "<br>点数量："+formatNum(directionByType[d.key][4]))
                        .style("left", (d3.event.pageX) + "px")
                        .style("top", (d3.event.pageY - 28) + "px");
                    })
                .append("path")
          ...
          myViolinDirection.append("rect")
              .attr("fill","black")
              .attr("x",x.bandwidth()/2-2)  // 矩形的 (x,y) 是右上角
              .attr("y",function (d) {
                  return y(directionByType[d.key][6])
              })
              .attr("height", function (d) {
                  return y(directionByType[d.key][5]) - y(directionByType[d.key][6]);
              })
              .attr("width", 4);

          myViolinDirection.append("circle")
              .attr("cx", x.bandwidth()/2 )
              .attr("cy", function (d) {
                  return y(directionByType[d.key][0])
              })
              .attr("r", 3)
              .style("fill", "red")
              .attr("stroke", "red")
...
function distanceViolin(div_vis,type,dis_i,distance_interval){
    // distance_vis 是一个div jQuery对象，用来存放小提琴图，type：2, 3, 4，dis_i：表示距离范围，如1
    // distance_group {} : [["zone", "electrodes", "h2", "nwd", "wd", "distance"]]
    // set the dimensions and margins of the graph
    ...
    let min_y = data_min > 0 ? 0 : 0.8 * data_min - 0.2 < -1 ? -1 : 0.8 * data_min - 0.2;  // 0 : data_min - (1 + data_min) * 0.2
      ...
      var histogram = d3.histogram()
            .domain(y.domain())
            .thresholds(y.ticks(violin_bins_slider2.getValue()))    // Important: how many bins approx are going to be made? It is the 'resolution' of the violin plot
            .value(d => d);
      ...
      var sumstat = d3.nest()  // nest function allows to group the calculation per level of a factor:分层地分组数组元素。
        ...
        .entries(distance_group[dis_i]); //返回一组键-值元组（其实就是一个字典）
      // What is the biggest number of value in a bin? We need it cause this value will have a width of 100% of the bandwidth.
      var maxNum = 0;
      for (i in sumstat ){
        allBins = sumstat[i].value;
      ...
      var myColorViolin = d3.scaleOrdinal()
        ...
        .range(d3.schemeSet2);
      var count=0;
      let distanceStatisticByType={};
      let myViolinDistance = svg.selectAll("myViolinDistance")
        .data(sumstat)
        .enter()        // So now we are working group per group
        .append("g")
          .attr("transform", function(d){
              ...
              pdata.sort((a,b)=>a-b);  // 一定要注意，排序是按照字符串首字母来的！！！！！！      中值、均值、最大值、最小值、数组长度、Q1、Q3
              distanceStatisticByType[d.key] = [d3.quantile(pdata,0.5),d3.mean(pdata),pdata[pdata.length - 1],pdata[0],pdata.length,d3.quantile(pdata,0.25),d3.quantile(pdata,0.75)];
              return("translate(" + x(d.key) +" ,0)")
          } ) // Translation on the right to be at the group position
        .on("mouseout", function (d) {
            div_tooltip.transition()
      ...
      distance_group_static[dis_i] = distanceStatisticByType;
      if($('#violin_status2').bootstrapSwitch("state")) {
            myViolinDistance.on("mouseover", function (d) {
                    ...
                    div_tooltip.html("中值："+formatNum(distanceStatisticByType[d.key][0])+
                        ...
                        "<br>点数量："+formatNum(distanceStatisticByType[d.key][4]))
                        .style("left", (d3.event.pageX) + "px")
                        .style("top", (d3.event.pageY - 28) + "px");
                    })
                .append("path")
          ...
          myViolinDistance.append("rect")
              .attr("fill","black")
              .attr("x",x.bandwidth()/2-2)  // 矩形的 (x,y) 是右上角
              .attr("y",function (d) {
                  return y(distanceStatisticByType[d.key][6])
              })
              .attr("height", function (d) {
                  return y(distanceStatisticByType[d.key][5]) - y(distanceStatisticByType[d.key][6]);
              })
              .attr("width", 4);

          myViolinDistance.append("circle")
              .attr("cx", x.bandwidth()/2 )
              .attr("cy", function (d) {
                  return y(distanceStatisticByType[d.key][0])
              })
              .attr("r", 3)
              .style("fill", "red")
              .attr("stroke", "red")
